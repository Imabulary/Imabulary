...
class SetScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final futureQuizStatuses =
        ref.watch(quizRepositoryProvider).fetchQuizStatuses();

    return Layout(
      SingleChildScrollView(
        child: FutureBuilder(
            future: futureQuizStatuses,
            builder: (context, asyncQuizStatuses) {
              return Column(
                children: [
                  Button(
                  ...
                  flashcards.when(data: (flashcards) {
                    final studiedFlashcards =
                        flashcards.result.where((element) {
                      return element.quizStatus?.name ==
                          QuizStatuses.mastered.name;
                    });
                    final notStudiedFlashcards =
                        flashcards.result.where((element) {
                      return element.quizStatus?.name ==
                          QuizStatuses.not_studied.name;
                    });
                    final learningFlashcards =
                        flashcards.result.where((element) {
                      return element.quizStatus?.name ==
