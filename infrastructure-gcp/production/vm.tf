...
resource "google_service_account" "artifact_registry_sa" {
  account_id   = "prod-vm-registry-pull-sa"
...
resource "google_artifact_registry_repository_iam_member" "artifact_registry_pull" {
  ...
  repository = google_artifact_registry_repository.imabulary-prod-repo.name
  role       = "roles/artifactregistry.reader"
  member     = "serviceAccount:${google_service_account.artifact_registry_sa.email}"
  depends_on = [ google_artifact_registry_repository.imabulary-prod-repo ]
}
# External IP
resource "google_compute_address" "vm-ip" {
  name         = "prod-vm-reserved-ext-ip"
  region       = "us-central1"
}
# VM Instance
resource "google_compute_instance" "docker_vm" {
  ...
  network_interface {
    ...
    subnetwork = module.vpc.subnets["us-central1/prod-vm-subnet"].id
    access_config {
